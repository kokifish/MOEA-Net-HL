...
class SFN:
    ...
    def generate_host_randomly(self):
        ...
        if(self.host_count >= N):
            ...
            return False
        self.host_l = np.random.choice(N, self.host_count, replace=False).tolist()
        return True
    ...
    def set_ori_capacity_by_degree2betweeness(self):
        ...
        dv = {n : d**self.ALPHA_D/d_sum * self.RATIO for n, d in dv}#uniformize: sum(dv)=1
    ...
    def recal_loop(self,G:nx.graph):#G: the Graph after removing some nodes
        ...
        bc = nx.betweenness_centrality_subset(G,new_host,new_host,normalized=False)
        ...
        if(new_b_sum <= 1e-100):
            ...
            return 0, 0
        bc.update({n: b/new_b_sum for n,b in bc.items()})# sometimes new_b_sum==0
    ...
    def usep_max(self):
        ...
        return max(usep_dict.items(), key=lambda x:x[1])#node id  of usep max

    # average closeness_centrality of host 
    #接近中心性，到其余所有节点的平均距离的倒数 #计算的是所有节点
    def cal_avg_closeness_centrality(self):
        d = nx.closeness_centrality(self.G)#dict: {node_id: closeness_centrality}
        d = {k:v for k,v in d.items() if k in self.host_l}#only care about the host
    ...
    def cal_assortativity_coefficient(self):
        ...
        # M = len(new_host)
        # for i in new_host:
        #     bc[i] += M-1# cal betweenness of sender and receiver ###
...
if __name__ == "__main__":
    ...
    # print(nx.info(G))
    sfn = SFN(G, 50)
    sfn.host_l = [11, 22, 33, 42, 44, 47, 52, 56, 57, 58, 69, 77, 84, 85, 88, 104, 106, 118, 127, 129, 146, 162, 175, 184, 189, 196, 199, 210, 211, 215, 216, 225, 227, 228, 238, 239, 241, 243, 250, 254, 257, 260, 263, 272, 274, 277, 285, 290, 293, 294]
    start_time = time.process_time()
    sfn.update()
    print("[CN_Bsaic Test] update time used:", time.process_time() - start_time)
    ########################################################
    start_time = time.process_time()
    arrGlobalComEffi = sfn.cal_GlobalComEffi()
    print("[CN_Bsaic cal_GlobalCommunicactionEfficiency] time used:", time.process_time() - start_time)
    print("arrGlobalComEffi", arrGlobalComEffi, arrGlobalComEffi.shape, type(arrGlobalComEffi)) #, arrGlobalComEffi.shape
    ########################################################

    # print('self.ALPHA_R:', sfn.ALPHA_R, 'self.ALPHA_D:', sfn.ALPHA_D, 'self.RATIO:', \
    #     sfn.RATIO, 'self.lambda_c:', sfn.lambda_c, 'self.b_sum:', sfn.b_sum, \
    #     'self.L0', sfn.L0,'host l:', sfn.host_l)
    # print("avg D:", sfn.cal_average_D(), sfn.cal_average_D_formula())
    # print('L0', sfn.cal_L0(), sfn.cal_L0_fast())
    # print('sfn.cal_R_2():', sfn.cal_R_2())
    # for i in range(20):#top n load nodes
    #     load_max_nid = sfn.find_load_max()
    #     sfn.G.remove_node(load_max_nid)
    # cc_g = nx.connected_components(sfn.G)
    # test_l = list(cc_g)
    # print('type(test_l)', type(test_l))
    # print(test_l)
    # k, usep = sfn.usep_max()
    # print('musp max node:', k, '; usep:', usep)
    # for i in range(10):
    #     print(i,' node usep:',sfn.G.node[i]['usep'], ' betweenness:',sfn.G.node[i]['betweenness'])
    # print('clustering:', sfn.cal_average_clustering())
    # start_time = time.process_time()
    # print('cal_average_D:', sfn.cal_average_D())
    # print('cal_average_D time: ', time.process_time() - start_time)

    # start_time = time.process_time()
    # print('cal_average_D_formula:', sfn.cal_average_D_formula())
    # print('cal_average_D_formula time: ', time.process_time() - start_time)

    # start_time = time.process_time()
    # print('cal_L0:', sfn.cal_L0())
    # print('cal_L0 time: ', time.process_time() - start_time)

    # start_time = time.process_time()
    # print('cal_L0_fast:', sfn.cal_L0_fast())
    # print('cal_L0_fast time: ', time.process_time() - start_time)
    # print('node[0] usep:', sfn.G.node[0]['usep'], 'node[0] ori_capacity:',sfn.G.node[0]['ori_capacity'])
    # sfn.draw_graph()
    ...
    #     sfn.host_l = []
    #     sfn.update()
    #     f1.append(sfn.lambda_c)
    #     f2.append(sfn.cal_R_2())
    # print('f1:', f1)
    # print('f2:', f2)

    # l = TimeLog['R_total']
    # print('R_total mean:', sum(l) / random_POP_SIZE)
    # l = TimeLog['cf']
    # print('cf mean:', sum(l) / random_POP_SIZE)

    # l = TimeLog['cc']
    # print('cc mean:', sum(l) / random_POP_SIZE)
    # l = TimeLog['recal_total']
    # print('recal_total mean:', sum(l) / random_POP_SIZE)
    # l = TimeLog['recal_bet']
    # print('recal_bet mean:', sum(l) / random_POP_SIZE)
    # l = TimeLog['remove']
    # print('remove mean:', sum(l) / random_POP_SIZE)
    
    #########################################
    # R_total mean: 7.2234375
    # cf mean: 4.961458333333334
    # cc mean: 0.0010416666666666667
    # recal_total mean: 4.800520833333334
    # recal_bet mean: 4.66875
    # remove mean: 1.60625
    #########################################
